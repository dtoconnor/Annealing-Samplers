...
module SpinVectorO3Dynamics
    ...
    using ..SamplingTools: hj_info, add_noise, add_cross_talk, discrete_aggregation
    ...
    function O3_Bloch_model(theta_vector::Array{Float64, 1},
        ...
        # # callback to coonserve system properties
        # function g(resid,u,p,t)
        #     mx, my, mz = u[i1], u[i2], u[i3]
        #     dr = mx.^2 + my.^2 + mz.^2 .- 1.0
        #     resid[i1] = dr
        #     resid[i2] = dr
        #     resid[i3] = dr
        #
        # end
        # cb = DifferentialEquations.ManifoldProjection(g)
    ...
    function solve(h::Dict, J::Dict; anneal_time=100.0, run_time=0.0, # in nanoseconds
                    ...
                    initial_phi_state=Array{Float64, 1}(),
                    saveat=collect(0:0.01:1),  # points in normalized time
                    abstol=1e-9, reltol=1e-6, kwargs...)
        ...
        if open_system
            ...
            results = discrete_aggregation(spin_vectors, trajectories, collect(UpperTriangular(JS)))
        else
            if !Bloch
